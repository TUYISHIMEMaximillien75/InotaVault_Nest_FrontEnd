    ...
    <div className="min-h-screen bg-[#f8f5f0] flex items-center justify-center p-6">
      ...
      <div className="bg-white border border-gray-200 w-full max-w-md" style={{ fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <div className="px-8 py-10 flex flex-col items-center text-center gap-4">
          ...
          <div>
            <p className="font-bold text-gray-900 mb-1" style={{ fontFamily:"'Playfair Display',serif" }}>Error Loading Dashboard</p>
  ...
  return (
    <>
      <style>{`
        ...
        .db-f1{animation:dbFadeUp .45s .04s both} .db-f2{animation:dbFadeUp .45s .10s both}
        .db-f3{animation:dbFadeUp .45s .16s both} .db-f4{animation:dbFadeUp .45s .22s both}
        .db-f5{animation:dbFadeUp .45s .28s both} .db-f6{animation:dbFadeUp .45s .34s both}
        ...
        .song-row{transition:background .15s} .song-row:hover{background:#faf9f7}
        .rep-row{transition:background .15s}  .rep-row:hover{background:#faf9f7}
        .qa-card{transition:transform .2s,box-shadow .2s}
        .qa-card:hover{transform:translateY(-3px);box-shadow:0 8px 24px rgba(0,0,0,.08)}
        .bar-fill{background:linear-gradient(90deg,#dc2626,#f87171);transition:width .6s cubic-bezier(.4,0,.2,1)}
      `}</style>

      <div className="db-body bg-[#f8f5f0] min-h-screen p-4 sm:p-6 lg:p-8">
        <div className="max-w-6xl mx-auto space-y-5 sm:space-y-7">

          {/* ═══ HEADER ═══ */}
          <div className="db-f1 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold text-red-500 uppercase tracking-[.22em] mb-1.5">Dashboard</p>
              <h1 className="db-display text-2xl sm:text-3xl lg:text-4xl font-black text-gray-900 leading-tight">
                {user?.name || "Musician"}<span className="text-red-600 italic">.</span>
              </h1>
              <p className="db-body text-gray-400 text-sm mt-1 truncate max-w-xs">{user?.email}</p>
            </div>
            <div className="flex gap-2 shrink-0">
              <NavLink to="/dashboard/upload"
                className="flex items-center gap-2 px-4 py-2.5 bg-red-600 text-white text-xs font-semibold uppercase tracking-wider hover:bg-red-700 transition-colors shadow-[0_4px_16px_rgba(220,38,38,.25)]">
                <Upload size={14} /> Upload
              </NavLink>
              <NavLink to="/dashboard/create_repertoires"
                className="flex items-center gap-2 px-4 py-2.5 bg-gray-950 text-white text-xs font-semibold uppercase tracking-wider hover:bg-gray-800 transition-colors">
                <Plus size={14} /> Repertoire
              </NavLink>
            </div>
          </div>

          {/* ═══ PRIMARY STATS — 2 cols on mobile, 4 on lg ═══ */}
          <div className="db-f2 grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
            {[
              ...
              <div key={label} className={`stat-card bg-white border border-gray-200 border-l-[3px] ${accent} p-4 sm:p-5 flex items-center gap-3`}>
                <div className={`w-9 h-9 sm:w-11 sm:h-11 ${iconBg} flex items-center justify-center shrink-0`}>
                  <Icon size={18} className={iconColor} />
                </div>
                <div>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[.15em] leading-none">{label}</p>
                  <p className="db-display text-xl sm:text-2xl font-black text-gray-900 leading-none mt-1">{value}</p>
                </div>
              </div>
            ))}
          </div>

          {/* ═══ MEDIA BREAKDOWN — 3 cols always ═══ */}
          <div className="db-f3 grid grid-cols-3 gap-2 sm:gap-3">
            {[
              ...
              <div key={label} className={`bg-white border ${border} flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3`}>
                <div className={`w-8 h-8 sm:w-9 sm:h-9 ${bg} flex items-center justify-center shrink-0`}>
                  <Icon size={15} className={color} />
                </div>
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[.1em] leading-none">{label}</p>
                  <p className="db-display text-lg sm:text-xl font-black text-gray-900 leading-none mt-0.5">{value}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="db-f4 bg-white border border-gray-200">
            <div className="px-4 sm:px-6 py-4 border-b border-gray-100 flex items-center gap-2">
              ...
              <h2 className="db-display font-bold text-gray-900 text-sm sm:text-base">Audience Engagement</h2>
              <span className="ml-auto text-[10px] font-bold text-gray-400 uppercase tracking-[.18em]">All-time</span>
            </div>

            {/* Metric trio — stacks to 1 col on xs, 3 on sm+ */}
            <div className="grid grid-cols-1 sm:grid-cols-3 sm:divide-x divide-gray-100 border-b border-gray-100">
              {[
                {icon:Heart,         label:"Likes",    value:totalLikes,    color:"text-rose-500",   bg:"bg-rose-50"   },
                {icon:Eye,           label:"Views",    value:totalViews,    color:"text-blue-500",   bg:"bg-blue-50"   },
                {icon:MessageCircle, label:"Comments", value:totalComments, color:"text-violet-500", bg:"bg-violet-50" },
              ].map(({icon:Icon, label, value, color, bg}, i) => (
                <div key={label} className={`flex items-center gap-3 px-4 sm:px-5 py-4 ${i > 0 ? "border-t sm:border-t-0 border-gray-100" : ""}`}>
            ...
            {/* Top songs table — scroll on mobile */}
            {topSongs.length > 0 ? (
              <>
                  ...
                  <table className="w-full text-sm min-w-[420px]">
                    <thead>
                      <tr className="text-[10px] uppercase tracking-[.14em] text-gray-400 border-b border-gray-100">
                        <th className="px-4 sm:px-6 py-2 text-left w-8">#</th>
                        ...
                        return (
                          <tr key={song.id} className="song-row border-t border-gray-50">
                            ...
                            <td className="px-4 py-3">
                              <p className="font-semibold text-gray-800 text-sm truncate max-w-[140px] sm:max-w-[180px]">{song.name}</p>
                              <p className="text-gray-400 text-[11px] truncate max-w-[140px] sm:max-w-[180px]">{song.artist}</p>
                            </td>
          ...
          {/* ═══ RECENT SONGS + REPERTOIRES — stack on mobile ═══ */}
          <div className="db-f5 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">

            {/* Songs — full width on mobile, 2/3 on lg */}
            <div className="lg:col-span-2 bg-white border border-gray-200">
              ...
              {recentSongs.length === 0 ? (
                <div className="flex flex-col items-center py-12 text-center px-6 gap-4">
                  <div className="w-12 h-12 bg-red-50 flex items-center justify-center">
                    <MicVocal size={22} className="text-red-300" />
                  </div>
                  <div>
                  ...
                  <table className="w-full text-sm min-w-[320px]">
                    <thead>
                      <tr className="text-[10px] uppercase tracking-[.14em] text-gray-400 bg-gray-50 border-b border-gray-100">
                        <th className="px-4 sm:px-6 py-2.5 text-left">Song</th>
                        ...
                        <tr key={song.id} className="song-row border-t border-gray-50">
                          <td className="px-4 sm:px-6 py-3.5">
                            <div className="flex items-center gap-2 sm:gap-3">
                              <div className="w-7 h-7 sm:w-8 sm:h-8 bg-red-600 flex items-center justify-center shrink-0">
                                <Music2 size={12} className="text-white" />
                              </div>
                              <div className="min-w-0">
                                <p className="font-semibold text-gray-800 text-sm truncate max-w-[110px] sm:max-w-[150px]">{song.name}</p>
                                <p className="text-gray-400 text-[11px] truncate max-w-[110px] sm:max-w-[150px]">{song.artist}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3.5 hidden sm:table-cell">
                            {song.category && (
                              <span className="inline-block px-2 py-0.5 text-[10px] font-bold bg-gray-100 text-gray-600 uppercase tracking-wide whitespace-nowrap">
                                {song.category}
                              </span>
                            )}
                          </td>
                          <td className="px-4 py-3.5 hidden md:table-cell">
                            <div className="flex items-center gap-1.5">
                              {song.audio_file && <Headphones size={12} className="text-green-500" />}
                              {song.video_file && <Film size={12} className="text-blue-500" />}
                              {song.pdf_sheet  && <FileMusic size={12} className="text-amber-500" />}
                            </div>
                          </td>
                          <td className="px-4 py-3.5 text-right text-gray-400 text-[11px] whitespace-nowrap">{fmtDate(song.createdAt)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {/* Repertoires — full width on mobile, 1/3 on lg */}
            <div className="bg-white border border-gray-200 flex flex-col">
              ...
              <div className="flex-1 divide-y divide-gray-50">
                {recentRepertoires.length === 0 ? (
                  <div className="flex flex-col items-center py-10 text-center px-5 gap-3">
                    <div className="w-11 h-11 bg-blue-50 flex items-center justify-center">
                  ...
                  recentRepertoires.map(rep => (
                    <div key={rep.id} className="rep-row px-4 sm:px-5 py-4 flex items-start gap-3 group">
                      <div className="w-7 h-7 bg-blue-600 flex items-center justify-center shrink-0 mt-0.5">
                        <BookOpen size={12} className="text-white" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-800 text-sm truncate">{rep.title}</p>
                        <div className="flex items-center justify-between mt-1 gap-2">
                          <span className="text-[11px] text-gray-400 truncate">{rep.event_type}</span>
                          <NavLink to={`/dashboard/edit_repertoire/${rep.id}`}
                            className="text-[11px] text-blue-500 hover:text-blue-700 font-semibold flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          ...
          {/* ═══ CATEGORIES ═══ */}
          {categories.length > 0 && (
            <div className="db-f6 bg-white border border-gray-200 p-4 sm:p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            {[
              ...
              <NavLink key={to} to={to}
                className={`qa-card group flex flex-col items-center justify-center gap-2 sm:gap-3 p-4 sm:p-5 border ${lightBg} text-center`}>
                <div className={`w-9 h-9 sm:w-10 sm:h-10 ${bg} ${hover} flex items-center justify-center transition-colors group-hover:scale-110 duration-200`}>
                  <Icon size={16} className="text-white" />
                </div>
                <span className={`text-[10px] sm:text-xs font-bold ${text} uppercase tracking-[.1em] leading-tight text-center`}>{label}</span>
              </NavLink>
            ))}
          </div>
